...
function areaVariableName(location) {
  location = location.toLowerCase()
  const toReplace = [' ', ',', '.', "'", '-']
  for (let c of toReplace) {
    location = location.replaceAll(c, '_')
  }
  return location
}
...
for (let area of Areas) {
  let areaId = `area_${areaVariableName(area.location)}`
  create += `${areaId} = Area(name="${area.location}")\n`
  for (let boss of area.bosses) {
    create += `${areaId}.bosses.append(Boss(name="${boss.name}", defeated=False))\n`
  }
  create += `user.areas.append(${areaId})\n\n`
}
console.log(create)
